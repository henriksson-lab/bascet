...
impl FASTQRecordParser {
    pub fn new() -> Self {
        FASTQRecordParser { inner_cursor: 0 }
    }
}

impl Parse<ArenaSlice<u8>> for FASTQRecordParser {
    ...
    where
        C: bascet_core::Composite
            + Default
            + FromParsed<A, Self::Output>
            + FromBacking<Self::Output, <C as bascet_core::Composite>::Backing>,
    {
        ...
        composite_record.from(&fastq_record);
            ...
            + FromBacking<Self::Output, <C as bascet_core::Composite>::Backing>,
    {
        ...
        let (pos_newline_combined, head_len) =
            match (iter_tail.next(), iter_tail.next(), iter_tail.next()) {
                (Some(n0), Some(n1), Some(n2)) => match iter_head.next() {
                    Some(h0) => ([n0, n1, n2, tail_len + h0], h0 + 1),
                    _ => return ParseStatus::Error(()),
                },
                (Some(n0), Some(n1), None) => match (iter_head.next(), iter_head.next()) {
                    (Some(h0), Some(h1)) => ([n0, n1, tail_len + h0, tail_len + h1], h1 + 1),
                    _ => return ParseStatus::Error(()),
                },
                (Some(n0), None, None) => {
                    match (iter_head.next(), iter_head.next(), iter_head.next()) {
                        (Some(h0), Some(h1), Some(h2)) => {
                            ([n0, tail_len + h0, tail_len + h1, tail_len + h2], h2 + 1)
                        }
                        _ => return ParseStatus::Error(()),
                    }
                }
...
impl FASTQRecord {
    unsafe fn from_raw(
        ...
        FASTQRecord {
            ...
            sequence: std::mem::transmute(seq),
            quality: std::mem::transmute(qal),
