...
impl CombinatorialBarcode8bp {
    ...
    pub fn add_pool(
        &mut self,
        poolname: &str,
        pool: CombinatorialBarcodePart8bp
    ) {
    ...
    pub fn add_bc(
        ...
        let pool: &mut CombinatorialBarcodePart8bp = self.pools.get_mut(*pool_index).expect("get pool fail");
...
impl CombinatorialBarcodePart8bp {
    ...
    pub fn detect_barcode(
        ...
        for current_pos in self.all_test_pos.iter() {
            ...
            let (bc_index, bc_distance) = HotEncodeATCGN::closest_by_hamming_u32(current_seq, self.barcode_seq_list.as_slice());
