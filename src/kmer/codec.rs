...
impl<const K: usize> Codec<K> {
    ...
    pub unsafe fn encode(
        ...
        for i in (0..Self::KMER_SIZE as usize).step_by(4) {
            let idx = (kmer[i] - b'A') as usize + 
                    ((kmer[i + 1] - b'A') as usize * NT4_DIMSIZE) +
                    ((kmer[i + 2] - b'A') as usize * NT4_DIMSIZE * NT4_DIMSIZE) +
                    ((kmer[i + 3] - b'A') as usize * NT4_DIMSIZE * NT4_DIMSIZE * NT4_DIMSIZE);
            encoded = (encoded << 8) | u128::from(NT4_LOOKUP[idx]);
        }
        
        // Handle any remaining nucleotides (if KMER_SIZE is not divisible by 4)
        for i in ((Self::KMER_SIZE as usize / 4) * 4)..Self::KMER_SIZE as usize {
    ...
    pub unsafe fn encode_str(
        ...
        for i in (0..Self::KMER_SIZE as usize).step_by(4) {
            let idx = (bytes[i] - b'A') as usize + 
                    ((bytes[i + 1] - b'A') as usize * NT4_DIMSIZE) +
                    ((bytes[i + 2] - b'A') as usize * NT4_DIMSIZE * NT4_DIMSIZE) +
                    ((bytes[i + 3] - b'A') as usize * NT4_DIMSIZE * NT4_DIMSIZE * NT4_DIMSIZE);
            encoded = (encoded << 8) | u128::from(NT4_LOOKUP[idx]);
        }
