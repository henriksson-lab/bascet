...
pub fn create_random_readers(
    ...
    for p in &params.path_in {
        let read_thread = match crate::fileformat::detect_shard_format(&p) {
            DetectedFileformat::TIRP => create_random_reader_thread(
                &p,
                &thread_pool_read,
                &rx_readcell,
                &tx_data,
            ...
            _ => {
                anyhow::bail!(
                    "Input file format for {} not supported for reading of specific cells",
                    p.display()
                )
            }
        };
...
pub fn create_stream_readers(
    ...
    for p in path_in {
        let read_thread = match crate::fileformat::detect_shard_format(&p) {
            DetectedFileformat::PairedFASTQ => create_stream_reader_thread(
                &p,
                &thread_pool_read,
                &tx_data,
            ...
            _ => {
                anyhow::bail!(
                    "Input file format for {} not supported for streaming all content",
                    p.display()
                )
            }
        };
...
fn get_list_of_all_cells(params: &Arc<TransformFile>) -> Vec<CellID> {
    //Get full list of cells, or use provided list. possibly subset to cells present to avoid calls later?
...
where
    R: ReadPairReader + ShardCellDictionary,
{
    ...
    thread_pool.execute(move || {
        ...
        let list_cells = reader
            .get_cell_ids()
            .expect("Could not get list of cells for input file");
        ...
        while let Ok(Some(cell_id)) = rx_readcell.recv() {
            ...
            if list_cells.contains(&cell_id) {
                let list_reads = reader
                    .get_reads_for_cell(&cell_id)
                    .expect("Failed to get reads from input file");
