...
pub fn detect_shard_format(p: &PathBuf) -> DetectedFileformat {
    let p_string = p
        .file_name()
        .expect("cannot convert OS string when detecting file format")
        .to_string_lossy();
    ...
    } else if p_string.ends_with(".R1.fq.gz")
        | p_string.ends_with(".R1.fastq.gz")
        | p_string.ends_with(".R1.fq")
        | p_string.ends_with(".R1.fastq")
    {
        DetectedFileformat::PairedFASTQ
    } else if p_string.ends_with(".fq.gz")
        | p_string.ends_with(".fastq.gz")
        | p_string.ends_with(".fq")
        | p_string.ends_with(".fastq")
    {
...
pub fn verify_input_fq_file(path_in: &PathBuf) -> anyhow::Result<()> {
    ...
    if file_format == DetectedFileformat::SingleFASTQ
        || file_format == DetectedFileformat::PairedFASTQ
    {
...
pub fn get_fq_filename_r2_from_r1(path: &PathBuf) -> anyhow::Result<PathBuf> {
    let p_string = path
        .file_name()
        .expect("cannot convert OS string when detecting file format")
        .to_string_lossy();
    if p_string.ends_with(".R1.fq.gz") {
        anyhow::Ok(PathBuf::from(replace_file_ending(
            path,
            ".R1.fq.gz",
            ".R2.fq.gz",
        )))
