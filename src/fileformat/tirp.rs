...
impl ConstructFromPath<TirpBascetShardReader> for TirpBascetShardReaderFactory {
    fn new_from_path(&self, fname: &PathBuf) -> anyhow::Result<TirpBascetShardReader> {
...
impl TirpBascetShardReader {
    pub fn new(fname: &PathBuf) -> anyhow::Result<TirpBascetShardReader> {
        ...
        if !index_path.exists() {
            bail!(
                "Cannot find tabix .tbi-file for {}; is this really a TIRP file?",
                fname.display()
            );
        }
    ...
    fn extract_to_outdir(
        ...
        if get_fastq {
            ...
            for line in self.tabix_reader.records() {
                ...
                let rec_r1 =
                    FastqRecord::new(Definition::new(format!("r{}", num_read), ""), rp.r1, rp.q1);
                let rec_r2 =
                    FastqRecord::new(Definition::new(format!("r{}", num_read), ""), rp.r2, rp.q2);
...
pub fn parse_tirp_readpair(line: &Vec<u8>) -> ReadPair {
    //Structure of each line:
    //cell_id  1   1   r1  r2  q1  q2 umi
...
pub fn parse_tirp_readpair_with_cellid(line: &[u8], //Vec<u8>,
) -> (Vec<u8>, ReadPair) {
    //Structure of each line:
    //cell_id  1   1   r1  r2  q1  q2 umi
...
impl ConstructFromPath<TirpStreamingReadPairReader> for TirpStreamingReadPairReaderFactory {
    fn new_from_path(&self, fname: &PathBuf) -> anyhow::Result<TirpStreamingReadPairReader> {
        ///////// maybe anyhow prevents spec of reader?
...
impl ShardFileExtractor for TirpStreamingShardExtractor {
    /// can make it for any readpairstreamer. TODO generalize this
...
impl ConstructFromPath<BascetTIRPWriter> for BascetTIRPWriterFactory {
    fn new_from_path(&self, fname: &PathBuf) -> anyhow::Result<BascetTIRPWriter> {
        ///////// maybe anyhow prevents spec of reader?
