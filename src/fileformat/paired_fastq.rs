...
impl ConstructFromPath<BascetPairedFastqWriter> for BascetPairedFastqWriterFactory {
    fn new_from_path(&self, fname: &PathBuf) -> anyhow::Result<BascetPairedFastqWriter> {
...
impl BascetPairedFastqWriter {
    fn new(path: &PathBuf) -> anyhow::Result<BascetPairedFastqWriter> {
        ...
        if let Some(last_pos) = last_pos {
            ...
            let out_buffer_r2 =
                File::create(&path_r2).expect("Failed to create fastq.gz output file");
...
impl ConstructFromPath<PairedFastqStreamingReadPairReader>
    for PairedFastqStreamingReadPairReaderFactory
{
