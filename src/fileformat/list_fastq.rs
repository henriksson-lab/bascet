...
impl ConstructFromPath<ListFastqReader> for ListFastqReaderFactory {
    fn new_from_path(&self, fname: &PathBuf) -> anyhow::Result<ListFastqReader> {
...
impl ReadPairReader for ListFastqReader {
    fn get_reads_for_cell(&mut self, cell_id: &String) -> anyhow::Result<Arc<Vec<ReadPair>>> {
        ...
        let mut forward_file =
            open_fastq(&self.records.get(cell_id).unwrap().r1).expect("Could not open fastq R1"); //TODO detect if fastq or fasta by first character
        let mut reverse_file =
            open_fastq(&self.records.get(cell_id).unwrap().r2).expect("Could not open fastq R2");
