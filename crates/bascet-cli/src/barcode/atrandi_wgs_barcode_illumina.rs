...
impl crate::barcode::Chemistry for DebarcodeAtrandiWGSChemistryIllumina {
    ...
    fn detect_barcode_and_trim<'a>(
        ...
        if score >= 0 {
            ...
            let overlap = compute_read_overlap(r2_seq, r1_seq);

            //Get subset ranges from trimming
            let trim_ranges =
                get_trimmed_ranges(self.barcode.trim_bcread_len, r2_seq, r1_seq, overlap);
...
pub fn compute_overlap_from_r1(
    r1_seq: &[u8],
    r2_seq: &[u8],
    tryoverlap_pos_r1: usize,
...
impl<'a> OverlapSearch<'a> {
    ...
    fn findmin(&self, r1_seq_hotencoded: &[u8]) -> Option<(usize, usize)> {
        //Return (position, how much left)
        let tryoverlap_seq_rc = revcomp_n(&self.tryoverlap_seq);
        if let Some(pos) = bithack_findmin_final(r1_seq_hotencoded, tryoverlap_seq_rc.as_slice()) {
...
fn compute_read_overlap(r1_seq: &[u8], r2_seq: &[u8]) -> usize {
...
fn get_trimmed_ranges(
    ...
    let mut r1_to = len1;
    if len2 < ov {
        if len1 + len2 < bc_len + ov {
            // to=len1+len2-ov < bc_len=from
...
fn bithack_findmin_final(allchar: &[u8], scanfor: &[u8]) -> Option<usize> {
    ...
    for (curpos, inp) in allchar_first_even16.windows(16).enumerate() {
        //keep enumerate. oddly faster (re-check later)
        ...
        if oneret > 5 {
            ...
            if total_match > 14 {
                // TODO set to 14 later. now disabled
                //println!("bye");
                return Some(curpos);
            }
    ...
    for curpos in (allchar.len() - 16)..(allchar.len() - 8) {
        ...
        if match1 > 5 {
            ...
            if total_match > 6 + (remain_len as u8) {
                // TODO set to 6 later for 2bp mismatch. now disabled
                //println!("bye");
                return Some(curpos);
            }
