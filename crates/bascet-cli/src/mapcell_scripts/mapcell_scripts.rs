    ...
    fn construct(&self) -> Arc<Box<dyn MapCellFunction>> {
        match self {
            MapCellFunctionConstuctor::ShellScriptConstructor(content) => {
                ...
                let script = MapCellFunctionShellScript::new_from_reader(&mut read_content)
                    .expect("Failed to instanciate script");
...
fn get_preset_scripts() -> HashMap<String, MapCellFunctionConstuctor> {
    ...
    map.insert(
        "abricate".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("abricate.sh").to_vec()),
    );
    map.insert(
        "amrfinder".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("amrfinder.sh").to_vec()),
    );
    map.insert(
        "ariba".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("ariba.sh").to_vec()),
    );
    map.insert(
        "bakta".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("bakta.sh").to_vec()),
    );
    map.insert(
        "checkm".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("checkm.sh").to_vec()),
    );
    map.insert(
        "fastqc".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("fastqc.sh").to_vec()),
    );
    map.insert(
        "gecco".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("gecco.sh").to_vec()),
    );
    map.insert(
        "gtdbtk".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("gtdbtk.sh").to_vec()),
    );
    map.insert(
        "kmc_process_reads".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(
            include_bytes!("kmc_process_reads.sh").to_vec(),
        ),
    );
    map.insert(
        "kmc_process_contigs".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(
            include_bytes!("kmc_process_contigs.sh").to_vec(),
        ),
    );
    map.insert(
        "prokka".to_string(),
    ...
    map.insert(
        "spades".to_string(),
        MapCellFunctionConstuctor::ShellScriptConstructor(include_bytes!("spades.sh").to_vec()),
    );
    ...
    map.insert(
        "minhash_fq".to_string(),
        MapCellFunctionConstuctor::OtherConstructor(Arc::new(Box::new(
            super::minhash_fq::MapCellMinHashFQ {},
        ))),
    );
    /*
       map.insert(
           "countsketch_fq".to_string(),
           MapCellFunctionConstuctor::OtherConstructor(Arc::new(Box::new(
               super::countsketch_fq::MapCellCountSketchFQ {},
           ))),
       );
    */
