...
impl MinhashCodec {
    ...
    #[inline(always)]
    pub fn hash_packed_kmer(kmer: u64) -> u64 {
        //&[u8]
...
impl MinhashKMER {
    pub fn new(kmer: &[u8]) -> MinhashKMER {
    ...
    fn get_minhash_fq_one(
        ...
        while cur_reads < max_reads {
            if let Some(_line1) = readit.next() {
                ...
                for encoded in seq.as_bytes().windows(codec.kmer_size) {
                    ///////////////////////////////////////////// Note: this is horrible! overlapping conversions
