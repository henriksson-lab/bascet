...
impl MapCellFunctionShellScript {
    pub fn new_from_reader(
        ...
        {
            let mut script_file =
                File::create_new(&path_script).expect("Failed to create temp script file");
            let _ = std::io::copy(preset_script_code, &mut script_file)
                .expect("Failed to copy script to temp file");
        }
        let path_script = path_script
            .canonicalize()
            .expect("Failed to get full temp script path");

        let path_script_s = &path_script.clone().into_os_string().into_string().unwrap();
        info!("Extracted preset script to {:?}", &path_script_s);
...
impl MapCellFunction for MapCellFunctionShellScript {
    ...
    fn invoke(
        ...
        let input_dir =
            to_absolute_path(&input_dir).expect("Could not get absolute directory for input");
        let output_dir =
            to_absolute_path(&output_dir).expect("Could not get absolute directory for output");
        let path_script = to_absolute_path(&self.script_file)
            .expect("Could not get absolute directory for script");
        ...
        let run_output = process::Command::new("bash") // do we need path?
            ...
            .expect(
                format!(
                    "Could not spawn process in mapcell script {:?}",
                    &path_script
                )
                .as_str(),
            );
        ...
        let success = if let Some(last_line) = last_line {
            last_line == "MAPCELL-OK"
        } else {
            false
        };
...
pub fn extract_needed_files_to_directory_generalapi(
    ...
    if success {
        ...
        if missing_file_mode == MissingFileMode::Fail {
            panic!(
                "Failed extraction of {}; shutting down process, keeping temp files for inspection",
                cell_id
            );
        }
