...
impl FeaturiseKmcCMD {
    ...
    pub fn try_execute(&mut self) -> Result<()> {
        ...
        info!("Featurise has finished succesfully");
        Ok(())
    }
}
...
impl FeaturiseKMC {
    ...
    pub fn run(params: &Arc<FeaturiseParamsKMC>) -> anyhow::Result<()> {
        ...
        if params.path_tmp.exists() {
            ...
            anyhow::bail!(
                "Temporary directory '{}' exists already. For safety reasons, this is not allowed. Specify as a subdirectory of an existing directory",
                params.path_tmp.display()
            );
        } else {
