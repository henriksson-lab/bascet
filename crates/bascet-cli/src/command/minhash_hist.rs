...
impl MinhashHist {
    /// Run the algorithm
    pub fn run(params: &Arc<MinhashHist>) -> anyhow::Result<()> {
        ...
        let list_cells = if let Some(p) = &params.include_cells {
            ...
            for path_input in &params.path_input {
                let mut file_input =
                    ZipBascetShardReader::new(&path_input).expect("Failed to open input file");
                let mut cells_for_file = file_input
                    .get_cell_ids()
        ...
        for path_input in &params.path_input {
            ...
            let cells_for_file = file_input
                .get_cell_ids()
                .expect("Failed to get content listing for input file");
            let cells_for_file = cells_for_file
                .iter()
            ...
            for cell_id in cells_for_file {
                ...
                if file_input.has_cell(&cell_id) {
                    ...
                    if list_files.contains(&f1) {
                        let path_f1 = params
                            .path_tmp
                            .join(format!("cell_{}.minhash.txt", cur_file_id).to_string());
        ...
        info!("MinhashHist has finished succesfully");
        Ok(())
    }
}
