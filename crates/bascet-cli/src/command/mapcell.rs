    ...
    pub fn try_execute(&mut self) -> Result<()> {
        ...
        let script: Arc<Box<dyn MapCellFunction>> = if preset_name.starts_with("_") {
            ...
            if let Some(script) = crate::mapcell_scripts::get_preset_script(preset_name) {
                ...
                //Return error instead of just crashing
        ...
        let (num_threads_read, num_threads_write, num_threads_mapcell) =
            determine_thread_counts_mapcell(
        ...
        let params = mapcell::MapCell {
            path_in: self
                .path_in
                .as_ref()
                .expect("Input file was not provided")
                .clone(),
            path_tmp: self.path_tmp.clone(),
            path_out: self
                .path_out
                .as_ref()
                .expect("Output file was not provided")
                .clone(),
            script: script,
...
impl MapCell {
    /// Run the algorithm
    pub fn run(params: MapCell) -> anyhow::Result<()> {
        ...
        while let Ok(Some(cell_id)) = rx.recv() {
            ...
            for (file_path, &file_name) in list_output_files.iter().zip(names_in_zip.iter()) {
                ...
                let compression_mode = match mapcell_script.get_compression_mode(file_name) {
                    ...
                    CompressionMode::Default => zip::CompressionMethod::Deflated,
